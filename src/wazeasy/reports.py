...
def run_basic_report(df, start_date = None, end_date = None):
    ...
    plots.jams_per_day_per_level(df_filt)
    plots.hourly_tci_by_month(df_filt, dow = [0,1,2,3,4], group_name = 'Weekdays')
    plots.hourly_tci_by_month(df_filt, dow = [5,6], group_name = 'Weekends')
    plots.plot_year_to_year_tci(df_filt, 'length', start_date, end_date)
...
def run_geog_report(df, geographies, start_date = None, end_date = None):
    ...
    for geog, geog_data in geographies.items():
        ...
        m = plots.map_tci(df_filt, geog, 'length', gdf_area, start_date = start_date, end_date = end_date, dow = None)
        display(m)
        if plot_by_geography:
            plots.plot_tci_daily_spatial(df_filt, geog, 'length', region_name, start_date = start_date, end_date = end_date, dow = None)
            plots.hourly_tci_by_geog(df_filt, geog, 'length', region_name, 'Weekdays', start_date = start_date, end_date = end_date, dow = [0,1,2,3,4])
            plots.hourly_tci_by_geog(df_filt, geog, 'length', region_name, 'Weekends', start_date = start_date, end_date = end_date, dow = [5,6])